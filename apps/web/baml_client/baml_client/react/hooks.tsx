...
/**
 * A specialized hook for the AnalyzeVideo BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 * - video_name: string
 ...
 * - **Streaming Partial:** partial_types.VideoAnalysis
 * - **Streaming Final:** VideoAnalysis
 *
 * **Usage Patterns:**
 ...
 * const { data, error, isLoading, mutate } = useAnalyzeVideo({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useAnalyzeVideo({
 ...
 * ```
 */
export function useAnalyzeVideo(props: HookInput<'AnalyzeVideo', { stream: false }>): HookOutput<'AnalyzeVideo', { stream: false }>
export function useAnalyzeVideo(props?: HookInput<'AnalyzeVideo', { stream?: true }>): HookOutput<'AnalyzeVideo', { stream: true }>
export function useAnalyzeVideo(
  props: HookInput<'AnalyzeVideo', { stream?: boolean }> = {},
): HookOutput<'AnalyzeVideo', { stream: true }> | HookOutput<'AnalyzeVideo', { stream: false }> {
  let action = Actions.AnalyzeVideo;
  if (isStreamingProps(props)) {
    action = StreamingActions.AnalyzeVideo;
  }
