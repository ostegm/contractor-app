 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useProcessAudio({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useProcessAudio({
 *   stream: true | undefined,
 *   onStreamData: (partial) => console.log('Partial update:', partial),
 *   onFinalData: (final) => console.log('Final result:', final),
 *   onError: (err) => console.error('Error:', err),
 ...
 * ```
 */
export function useProcessAudio(props: HookInput<'ProcessAudio', { stream: false }>): HookOutput<'ProcessAudio', { stream: false }>
export function useProcessAudio(props?: HookInput<'ProcessAudio', { stream?: true }>): HookOutput<'ProcessAudio', { stream: true }>
export function useProcessAudio(
  props: HookInput<'ProcessAudio', { stream?: boolean }> = {},
): HookOutput<'ProcessAudio', { stream: true }> | HookOutput<'ProcessAudio', { stream: false }> {
  let action = Actions.ProcessAudio;
  if (isStreamingProps(props)) {
    action = StreamingActions.ProcessAudio;
  }
  return useBamlAction(action, props)
}
/**
 * A specialized hook for the ProcessVideo BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 * - video: InputFile
 ...
 *
 * **Return Type:**
 * - **Non‑streaming:** ProcessedVideo
 * - **Streaming Partial:** partial_types.ProcessedVideo
 * - **Streaming Final:** ProcessedVideo
 *
 * **Usage Patterns:**
 * 1. **Non‑streaming (Default)**
 *    - Best for quick responses and simple UI updates.
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useProcessVideo({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useProcessVideo({
 *   stream: true | undefined,
 *   onStreamData: (partial) => console.log('Partial update:', partial),
 *   onFinalData: (final) => console.log('Final result:', final),
 *   onError: (err) => console.error('Error:', err),
 ...
 * ```
 */
export function useProcessVideo(props: HookInput<'ProcessVideo', { stream: false }>): HookOutput<'ProcessVideo', { stream: false }>
export function useProcessVideo(props?: HookInput<'ProcessVideo', { stream?: true }>): HookOutput<'ProcessVideo', { stream: true }>
export function useProcessVideo(
  props: HookInput<'ProcessVideo', { stream?: boolean }> = {},
): HookOutput<'ProcessVideo', { stream: true }> | HookOutput<'ProcessVideo', { stream: false }> {
  let action = Actions.ProcessVideo;
  if (isStreamingProps(props)) {
    action = StreamingActions.ProcessVideo;
  }
  return useBamlAction(action, props)
}