...
BEGIN
    ...
    DECLARE
        ...
        thread5_id UUID := uuid_generate_v4();
    ...
    BEGIN
        ...
        INSERT INTO chat_threads (id, project_id, name, created_at, updated_at) VALUES
            ...
            (thread5_id, project1_id, 'Quick Updates', event_time_today, event_time_today + INTERVAL '3 minutes');
        ...
        -- Project 1 Events (Thread 5 - Quick Updates with Patch Example)
        INSERT INTO chat_events (thread_id, event_type, data, created_at) VALUES
            (thread5_id, 'UserInput', jsonb_build_object('message', 'I need to update the cost of the plumbing work to $3000.'), event_time_today),
            (thread5_id, 'PatchEstimateRequest', jsonb_build_object('patches', jsonb_build_array(
                jsonb_build_object(
            ...
            )), event_time_today + INTERVAL '1 minute'),
            (thread5_id, 'PatchEstimateResponse', jsonb_build_object('patch_results', jsonb_build_array(
                jsonb_build_object('success', true),
                jsonb_build_object('success', true)
            )), event_time_today + INTERVAL '2 minutes'),
            (thread5_id, 'AssisantMessage', jsonb_build_object('message', 'I''ve updated the plumbing costs to $2,600-$3,000 in the estimate. The total project cost will now be slightly higher to reflect this change.'), event_time_today + INTERVAL '3 minutes');
